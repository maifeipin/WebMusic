...
import { X, Save, Music2, Image as ImageIcon, Search, Loader2, Disc } from 'lucide-react';
import api, { updatePlaylist, getPlugins } from '../services/api';
...
export default function EditPlaylistModal({ isOpen, onClose, playlist, onSaved }: EditPlaylistModalProps) {
    ...
    useEffect(() => {
        if (playlist) {
            ...
            // Init Search Query with playlist name
    ...
    const handleAutoFill = async (nSong: NeteaseSong) => {
        ...
        // 1. Optimistic Fill (Cover Art Only)
        // We usually DON'T want to change the playlist name to a song name, 
        // so we only update the cover art.
                    ...
                    <div className={`flex flex-1 overflow-hidden ${hasPlugin ? 'flex-col md:flex-row' : ''}`}>
                        ...
                        <div className={`p-6 overflow-y-auto custom-scrollbar space-y-6 ${hasPlugin ? 'md:w-1/2 border-r border-gray-800' : 'w-full'}`}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-400 mb-1.5">Playlist Name</label>
                                    <input
                                        type="text"
                                        value={name}
                                ...
                                <div className="p-4 border-b border-gray-800 bg-gray-900">
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            placeholder="Search songs for cover art..."
                                        />
                                        <button
                                    ...
                                    <p className="text-gray-500 text-xs mt-2">
                                        Search for a song, then use its album art as your playlist cover.
                                    </p>
                                </div>
                                ...
                                <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                                            ...
                                            <div key={res.id} className="group p-2 rounded-lg hover:bg-gray-800 transition flex gap-3 items-center border border-transparent hover:border-gray-700">
                                                ...
                                                <div className="flex-1 overflow-hidden min-w-0">
                                                    ...
                                                    <div className="text-xs text-gray-400 truncate">
                                                        {(res.artists || res.ar || []).map(a => a.name).join(', ')}
                                                    </div>
                                                </div>
                                                <button
                                                    id={`autofill-${res.id}`}
                                    ...
                                    {searchResults.length === 0 && !searching && (
                                        <div className="text-center text-gray-500 py-12 flex flex-col items-center gap-2">
                                            ...
                                            <span className="text-xs">Search songs to pick a cover</span>
                                        </div>
                                    )}
                                </div>
                            </div>
