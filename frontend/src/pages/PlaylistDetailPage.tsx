...
import { getPlaylist, removeSongsFromPlaylist, sharePlaylist } from '../services/api';
import { usePlayer } from '../context/PlayerContext';
import { Play, ArrowLeft, Trash2, CheckSquare, Square, Edit2, Share2, Copy, ExternalLink, Lock, Clock, Music } from 'lucide-react';
...
export default function PlaylistDetailPage() {
    ...
    // Song Edit Modal
    const [editingSong, setEditingSong] = useState<any>(null); // For EditSongModal
    ...
    // Edit Song State (Already declared above)
        ...
        <div className="p-6 md:p-8 min-h-screen pb-32">
            ...
            <div className="flex items-start gap-6 mb-8 mt-2">
                <button
                    ...
                    className="p-2 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition mt-1 flex-shrink-0"
                >
                    <ArrowLeft size={24} />
                </button>

                {/* Cover Art */}
                <div className="w-40 h-40 bg-gray-800 rounded-xl overflow-hidden shadow-2xl flex items-center justify-center flex-shrink-0 border border-gray-700 bg-gradient-to-br from-gray-800 to-gray-900">
                    {playlist.coverArt ? (
                        <SmbImage
                            smbPath={playlist.coverArt}
                            alt={playlist.name}
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-end h-40 pb-1">
                    <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-white mb-2 truncate" title={playlist.name}>
                        {playlist.name}
                    </h1>
                    <div className="flex items-center gap-3 text-sm text-gray-400 mb-6">
                        <span className="text-gray-300 font-medium">Playlist</span>
                        <span>•</span>
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="px-8 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-full flex items-center gap-2 font-bold transition shadow-lg shadow-blue-900/40 hover:scale-105 active:scale-95"
                        >
                        ...
                        <button
                            ...
                            className="p-2.5 text-gray-300 hover:text-white hover:bg-white/10 border border-gray-700 hover:border-gray-500 rounded-full transition flex items-center gap-2"
                            title="Edit Playlist"
                        >
                            <Edit2 size={20} />
                        </button>
                        ...
                        <button
                            ...
                            className="p-2.5 text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 border border-emerald-500/20 hover:border-emerald-500/50 rounded-full transition"
                        ...
                        {selectedItems.length > 0 && (
                            <button
                                ...
                                className="px-4 py-2.5 bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20 rounded-full flex items-center gap-2 font-medium transition ml-auto"
                            >
                                <Trash2 size={18} />
