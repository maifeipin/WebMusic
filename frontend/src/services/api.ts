...
export const deleteMedia = (id: number, force = false, permanent = false) => api.delete(`/media/${id}?force=${force}&permanent=${permanent}`);
export const getDeletedFiles = (params: any = {}) => api.get('/media', { params: { ...params, onlyDeleted: true } });
export const restoreMedia = (id: number) => api.post(`/media/${id}/restore`);
export const batchRestoreMedia = (ids: number[]) => api.post('/media/batch-restore', ids);
...
export const deleteLyrics = (mediaId: number) =>
    api.delete<{ success: boolean; deleted: boolean }>(`/lyrics/${mediaId}`).then(r => r.data);
